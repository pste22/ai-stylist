...
- [~] P3-1: Swap fake catalog for real items via 1 affiliate source (Amazon PA-API / LTK / Rakuten) — *adapter built: `product_source.py` `AmazonSource` (PA-API → our schema incl `image_url`/tagged `affiliate_url`) + `get_source()` factory (env `PRODUCT_SOURCE`, graceful fallback to local). Normalization unit-tested (`test_product_source.py`, no network). `Stylist` uses the factory; web cards render real images. Remaining: add Associates credentials to `.env` + `pip install amazon-paapi` to go live.*
