...
- [~] P3-1: Swap fake catalog for real items via 1 affiliate source (Amazon PA-API / LTK / Rakuten) — *both Amazon stages built behind `ProductSource`: pre-API `CuratedAmazonSource` (`PRODUCT_SOURCE=curated`, seed SiteStripe items in `data/affiliate_products.json`, `amazon_affiliate_url()` builds links from ASIN+tag) for the launch→first-3-sales phase; post-API `AmazonSource` (`PRODUCT_SOURCE=amazon`) flips on automatically once keys unlock. `get_source()` factory + graceful fallback; 28 tests pass. Remaining: seed real SiteStripe products + drive the first 3 sales.*
